...
# Окно ввода API-ключа (возвращено)
...
def call_grok(messages, use_tools=False):
    ...
    try:
        ...
        if r.status_code == 429:
            raise Exception("Лимит запросов исчерпан. Подождите или пополните баланс в https://console.x.ai/")
        elif r.status_code == 401 or r.status_code == 403:
            raise Exception("Неверный или просроченный API-ключ. Проверьте ключ в https://console.x.ai/")
        elif r.status_code == 503:
            raise Exception("Сервер Grok временно недоступен. Попробуйте позже.")
