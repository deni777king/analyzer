...
import pyperclip  # только эта строка добавлена для копирования
...
# Все функции без изменений (оставил как было, не трогал)
# SITE_SUMMARY_PROMPT, CANDIDATE_PROMPT, FINAL_REPORT_PROMPT и все остальные функции остаются точно такими же

# ... (вставь сюда все твои функции: fetch_site_profile, browse_page, build_url_variants, normalize_root_url, get_domain_key, clean_text, count_internal_links, tokenize, jaccard_similarity, cosine_similarity, compare_profiles, is_blocked_domain, summarize_profile, extract_candidate_urls, call_mistral, complete_with_tools, get_site_outline, get_candidate_domains, verify_competitors, build_final_report, build_validation_rows, run_full_analysis, render_validation_table, rerun_competitors_only, replace_section, build_section_5, build_section_7, build_section_8)
...
# КНОПКА КОПИРОВАНИЯ ДОМЕНА КОНКУРЕНТА (добавлена только сюда, как ты просил)
if st.session_state.verified_competitors:
    ...
    selected = st.selectbox("Выбери домен", competitors, key="copy_domain_select")
    if st.button("Копировать чистый домен"):
        pyperclip.copy(selected)
        st.success(f"Скопировано в буфер: {selected}")
